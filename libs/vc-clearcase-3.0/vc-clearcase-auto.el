...
(autoload (quote vc-clearcase-checkout-directory) "vc-clearcase" "\
...
(autoload (quote vc-clearcase-checkin-directory) "vc-clearcase" "\
...
(easy-menu-define clearcase-global-menu clearcase-global-menu-map "Menu for the extra (non-file related) ClearCase functionality" (quote ("ClearCase" ["Checkout directory..." vc-clearcase-checkout-directory t] ["Checkin directory..." vc-clearcase-checkin-directory t] "----" ["Start dynamic view..." vc-clearcase-start-view t] ["Edit Configspec..." vc-clearcase-edcs t] ["Update snapshot view..." vc-clearcase-update-view t] ["List Checkouts..." vc-clearcase-list-checkouts t] ["List View Private Files..." vc-clearcase-list-view-private-files t] ["Label diff report..." vc-clearcase-label-diff-report t] "----" ["Report bug in vc-clearcase..." vc-clearcase-report-bug t])))

(easy-menu-add-item menu-bar-tools-menu (quote nil) clearcase-global-menu "PCL-CVS")

(autoload (quote vc-clearcase-report-bug) "vc-clearcase" "\
...
(when (and (executable-find cleartool-program) (>= emacs-major-version 23)) (cond ((boundp (quote find-file-not-found-functions)) (add-hook (quote find-file-not-found-functions) (quote clearcase-file-not-found-handler))) ((boundp (quote find-file-not-found-hooks)) (add-hook (quote find-file-not-found-hooks) (quote clearcase-file-not-found-handler)))) (if (boundp (quote vc-handled-backends)) (unless (memq (quote CLEARCASE) vc-handled-backends) (setq vc-handled-backends (nconc vc-handled-backends (quote (CLEARCASE))))) (setq vc-handled-backends (quote (RCS CVS CLEARCASE)))))

;;;***

;;;### (autoloads (ucm-unlock-activity ucm-lock-activity ucm-checkin-activity
;;;;;;  ucm-browse-activity ucm-list-activities ucm-show-current-activity
;;;;;;  ucm-set-activity) "ucm" "ucm.el" (18752 60723))
...
(autoload (quote ucm-list-activities) "ucm" "\
Not documented
...
(defvar ucm-before-activity-lock-hook (quote nil) "\
...
(autoload (quote ucm-lock-activity) "ucm" "\
Lock ACTIVITY.  With prefix arg, mark it as obsolete.

\(fn ACTIVITY)" t nil)

(autoload (quote ucm-unlock-activity) "ucm" "\
